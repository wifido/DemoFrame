...
<form id="<%=FormUtils.mkFormId(PathConstants.PCOMP_TITLE_NAME)%>"
      action="<%=PathUtils.makePath(PathConstants.PCOMP_TITLE_CREATE_PATH)%>" method="get"
      onsubmit="return validate<%=PcompConstants.PCOMP_TITLE%>(this)">
    <div class="form-group" id="<%=FormUtils.mkFormGroupId(PathConstants.PCOMP_TITLE_NAME)%>">
        ...
        <input type="text" name="<%=PathConstants.PCOMP_TITLE_NAME%>" placeholder="请输入主题名称（不超过32个字）"
               class="form-control" id="<%=FormUtils.mkTextInputId(PathConstants.PCOMP_TITLE_NAME)%>"
               onblur="validate<%=PathConstants.PCOMP_TITLE_NAME%>(this)">
